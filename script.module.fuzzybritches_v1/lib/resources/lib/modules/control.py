...
def artPath():
    ...
    if theme in ['-', '']:
        return
    elif condVisibility('System.HasAddon(script.fuzzybritches_v1.artwork)'):
        return os.path.join(
            xbmcaddon.Addon('script.fuzzybritches_v1.artwork').getAddonInfo('path'),
            'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility(
        'System.HasAddon(script.fuzzybritches_v1.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.fuzzybritches_v1.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.fuzzybritches_v1metadata)'):
        return os.path.join(
            xbmcaddon.Addon('script.fuzzybritches_v1.metadata').getAddonInfo('path'),
