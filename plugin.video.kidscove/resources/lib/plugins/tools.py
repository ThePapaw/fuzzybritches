...
WALLPAPER_LIST = [ ("Wallpaper Abyss", "https://wall.alphacoders.com/"),
        ("Imgur Wallpapers 1920x1080 and up", "https://imgur.com/gallery/3jTX8/"),
		("BHM Pics", "http://www.bhmpics.com/"),
...
@route(mode="WALLPAPER", args=["url"])
def wallpaper_handler(url):
    try:
        site = ''
        if 'wallpaperlist' in url:
            names = []
            for item in WALLPAPER_LIST:
                the_title = 'wallpaper for %s' % (item[0].replace('_', ' ').capitalize())
                names.append(the_title)
            selected = xbmcgui.Dialog().select('Select Site',names)
            ...
            # If you add [COLOR] etc to the title stuff in names loop above, this will strip all of that out and make it usable here
            wallpaper_item = re.sub('\[.*?]','',names[selected]).replace('wallpaper for ', '').replace(' ', '_').lower()
            for item in WALLPAPER_LIST:
                if str(item[0]) == wallpaper_item:
                    site = item[1]
                    break
        else:
            for item in WALLPAPER_LIST:
